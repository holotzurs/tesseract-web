...
def _get_ocr_data(image: Image, language: str, include_ocr_bounding_boxes: bool = True):
    lang_code = Language.get(language).to_alpha3()
    text = pytesseract.image_to_string(image, lang=lang_code)
    
    json_ready_data = []
    if include_ocr_bounding_boxes:
...
def pdf_to_text(pdf_file_path: str, language="en", include_ocr_bounding_boxes: bool = True):
    all_page_results = []
    images = pdf_to_img(pdf_file_path)
    for _pg, img in enumerate(images):
        page_ocr_results = _get_ocr_data(img, language, include_ocr_bounding_boxes=include_ocr_bounding_boxes)
...
def _process_single_ocr_task(file_input: dict, upload_folder: str, supported_formats: list, include_ocr_bounding_boxes: bool = True) -> dict:
    result = {
    ...
    try:
        ...
        if file_extension == "pdf":
            page_results = pdf_to_text(temp_filepath, language, include_ocr_bounding_boxes=include_ocr_bounding_boxes) 
            full_text = []
            all_ocr_data = []
            if include_ocr_bounding_boxes:
...
def _process_ocr_job(job_id, files_payload, upload_folder, supported_formats):
    ...
    all_results = []
    job_overall_start_time = datetime.datetime.now(datetime.timezone.utc)

    try:
        for file_input in files_payload:
            include_ocr_bounding_boxes = file_input.get("include_ocr_bounding_boxes", True)
            single_file_result = _process_single_ocr_task(file_input, upload_folder, supported_formats, include_ocr_bounding_boxes=include_ocr_bounding_boxes)
        ...
        job_overall_end_time = datetime.datetime.now(datetime.timezone.utc)
...
def submit_async_ocr_job(files_payload, upload_folder, supported_formats):
    ...
    OCR_JOBS[job_id] = {
        ...
        "results": [],
        "overall_start_time": datetime.datetime.now(datetime.timezone.utc).isoformat(),
